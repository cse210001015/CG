...
int WINDOW_SIZE[2];
const char* diagramName = "Bresenham Line Drawing";
...
{
    glBegin(GL_POINTS);
    int dy = abs(end[1] - start[1]), dx = abs(end[0] - start[0]);
    int param = dy > dx ? 2*dx - dy : 2*dy - dx;
    ...
    while(steps--)
    {
        if(param <= 0)       param += dy > dx ? 2*dx : 2*dy;
        else
        {
            param += dy > dx ? 2*(dx - dy) : 2*(dy - dx);
